...
const store = new Vuex.Store({
	...
	getters: {
		//当前路由的中文名，取currentPath的最后一个
		currentPageNameCN(state) {
			return state.currentPath[state.currentPath.length - 1].title;
		},
		//处理字典成键值对格式
		dictionaries(state) {
			let obj = {};
	...
	actions: {
		asyncSelectDataStore({
			commit,
			state
		}) {

			state.asyncSelectData.store
		}
	}
});
