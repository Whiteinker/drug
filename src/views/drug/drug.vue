  ...
  <div class='page-drug'>
    <Card>
      <div slot="title"
        ...
        <div class="action">
          <Button type="warning"
            size="small"
            icon="archive"
            @click="openDrugFromLibraryModal">药品库</Button>
          <Button type="success"
            size="small"
            icon="plus-round"
            @click="openFormModal">增加</Button>
          <Button type="info"
            size="small"
            icon="loop"
            @click='getListData'>刷新</Button>
        </div>
      </div>
      <div class="common-search-box">
        <Select ref='asyncSelectStore'
          transfer
          v-model="searchFields.storeId"
          placeholder="请选择药店"
          filterable
          @on-change='storeChange'>
          <Option v-for="t in cacheStore"
            :value="t.value"
            :key='t.value'>{{t.label }}</Option>
        </Select>
        <inputTree style='display:inline-block'
          ref='inputTree2'
          :config='treeConfig'
          :model='searchFields.categoryName'
          @treeChange='inputTreeChange2'></inputTree>
        <Input v-model="searchFields.drugName"
        ...
        <div class="operation">
          <Button :type="listStyle=='grid'?'primary':'ghost'"
            size="small"
            icon="grid"
            @click='switchListStyle("grid")'></Button>
          <Button :type="listStyle=='navicon'?'primary':'ghost'"
            size="small"
            icon="navicon"
            @click='switchListStyle("navicon")'></Button>
        </div>
      </div>
      <Table v-if="listStyle=='navicon'"
        border
        class="margin-top-10"
      ...
      <div v-if="listStyle=='grid'"
        class="margin-top-10 card-list-box drug-card"
        style='margin-left:-24px;'>
        <Spin size="large"
          fix
          v-if="tableLoading"></Spin>
        <Card v-for='(t,i) in tableList'>
          <Tag :color="t.state==1?'blue':'default'"
            class='drug-tag'>
          ...
          <div class="text">
            <p>
              ...
              <span class='inventoryNum'
                style=''>库存：{{t.inventoryNum}}</span>
            </p>
            <h3>{{t.drugName}}</h3>
          ...
          <div class='btn-group'>
            <Button type="primary"
              ...
              size='small'
              icon="compose"
              @click='openFormModal({id:t.id})'>编辑</Button>
            <Poptip confirm
              transfer
              title='您确定要删除这条数据吗?'
              @on-ok='deleteListData({id:t.id})'>
              <Button type="error"
                size='small'
                shape="circle"
      ...
      <div class="common-page-box">
        <Poptip class='fl'
          ...
          <Button v-if="listStyle=='navicon'"
            type="error"
            icon="trash-a">批量删除</Button>
        </Poptip>
        <Page show-sizer
          show-elevator
    ...
    <Modal v-model="showFormModal"
      ...
      @on-cancel="closeHasInputTreeModal">
      <div class='common-form-box'>
        ...
        <Form ref="formModel"
          ...
          <FormItem label="药品图片："
            prop="photo"
            class='common-file-base64'
            style='display: block;'>
            <Avatar icon="upload"
              shape='square'
              size="large"
              :src='formModel.photo'></Avatar>
            <input type="file"
              title='选择上传文件'
              @change='changeFile($event,"photo")' />
          </FormItem>
          <FormItem label="药品名称："
            prop="drugName">
            <Input v-model="formModel.drugName"
              placeholder="请输入药品名称"></Input>
          </FormItem>
          <FormItem label="药店名称："
            prop="storeId">
          ...
          <FormItem label="药品分类："
            prop="categoryName">
            <inputTree ref='inputTree'
              :config='treeConfig'
              :model='formModel.categoryName'
              @treeChange='inputTreeChange'></inputTree>
          </FormItem>
          <FormItem label="药品分类id："
            prop="categoryId"
            class='hidden'>
            <Input v-model="formModel.categoryId"
              placeholder="请输入药品分类id"></Input>
          </FormItem>
          <FormItem label="生产商："
            prop="manufacturer">
            <Input v-model="formModel.manufacturer"
              placeholder="请输入生产商"></Input>
          </FormItem>
          <FormItem label="包装规格："
            prop="specification">
            <Input v-model="formModel.specification"
              placeholder="请输入包装规格"></Input>
          </FormItem>
          <FormItem label="使用剂量："
            prop="dosage">
            <Input v-model="formModel.dosage"
              placeholder="请输入使用剂量"></Input>
          </FormItem>
          <FormItem label="库存数量："
            prop="inventoryNum">
            <InputNumber :max="9999"
              :min="1"
              v-model="formModel.inventoryNum"
              placeholder="请输入库存数量"> </InputNumber>
          </FormItem>
          <FormItem label="单价："
            prop="unitPrice">
            <Input v-model="formModel.unitPrice"
          ...
          <FormItem label="药品状态："
            ...
            <Select transfer
              ...
              placeholder="请输入药品状态">
              <Option v-for="t in $store.state.dictionaries.drugState"
                :value="t.value"
                :key='t.value'>{{t.label }}</Option>
            </Select>
          </FormItem>
    ...
    <Modal class='page-drug-from-library-modal'
      v-model="showdrugFromLibraryModal"
      width='50%'
      title="药品库列表"
      @on-cancel="">
